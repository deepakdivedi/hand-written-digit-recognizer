...
def proc_user_img(img_file, model):
    ...
    plt.imshow(im)
    cv2.imwrite("E:\PROJECTS\DigitRecognizer\DATA\original_overlay.png",im) 
    cv2.imwrite("E:\PROJECTS\DigitRecognizer\DATA\output_image.png",blank_image) 
...
def load_digits_custom(img_file):
    ...
    for index,rect in enumerate(digits_rectangles):
        ...
        if index>0 and (index+1) % 10 == 0:
            start_class += 1
    cv2.imwrite("E:\PROJECTS\DigitRecognizer\DATA\_training_box_overlay.png",im)
    
    return np.array(train_data), np.array(train_target)

#------------------data preparation--------------------------------------------

TRAIN_MNIST_IMG = 'E:\PROJECTS\DigitRecognizer\DATA\digits.png' 
TRAIN_USER_IMG = 'E:\PROJECTS\DigitRecognizer\DATA\custom_train_digits.jpg'
TEST_USER_IMG = sys.argv[1];
...
print('Accuracy1: ',accuracy_score(y_test, preds))
...
model.train(X_train, y_train)
preds = model.predict(X_test)
print('Accuracy2: ',accuracy_score(y_test, preds))
